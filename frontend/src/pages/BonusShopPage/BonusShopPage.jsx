...
function BonusShopPage({ principal, authClient }) {
  ...
  const confirmExchange = async () => {
    if (selectedNft && porBalance >= Number(selectedNft.token_cost)) {
      ...
      nfts.push({ ...selectedNft, token_cost: Number(selectedNft.token_cost), id: Number(selectedNft.id)  })
          ...
          {!isFetching && (
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {nftBonuses
                .filter(bonus => selectedCategory === 'all' || bonus.category?.toLowerCase().includes(selectedCategory.toLowerCase()))
                .map((nft) => (
                    ...
                    <div className="p-4 flex flex-col justify-between">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="font-bold text-green-600">{Number(nft.token_cost)} Tokens</span>
                        </div>
                        <button
                          onClick={() => handleExchange(nft)}
                          disabled={porBalance < Number(nft.token_cost)}
                          className={`px-3 py-2 rounded-md text-sm font-medium ${porBalance >= Number(nft.token_cost)
                              ? "bg-green-600 text-white hover:bg-green-700"
                              : "bg-gray-200 text-gray-500 cursor-not-allowed"
                            }`}
                        >
                          {porBalance >= Number(nft.token_cost) ? "Exchange Tokens" : "Not Enough Tokens"}
                        </button>
                      </div>
                    </div>
                  </div>
              ...
              {!nftBonuses.filter(bonus => selectedCategory === 'all' || bonus.category?.toLowerCase().includes(selectedCategory.toLowerCase())).length && (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <p className="text-gray-600 mb-4">
              Are you sure you want to exchange <span className="font-bold">{Number(selectedNft.token_cost)} tokens</span> for "{selectedNft.title}" NFT?
            </p>
